...
int main_file(string const& filename)
{
    ...
    int64_t last_times = -1;
    while (m) {
        int64_t value = pow(m, 1.0/3.0);
        int64_t times = m/trunc(pow(value, 3.0));
        if (value > 10) {
            last_value = pow(value, 3.0);
            last_times = times;
            m -= times*last_value;
            k += last_times;
        } else {
    ...
    int64_t best_v = -1;
    int64_t best_k = -1;
    for (int v = 0; v < min(m,1000LL); v++) {
        int64_t curr_m = m - v;
        int64_t curr_k = k;
        while (curr_m) {
            int64_t value = pow(curr_m, 1.0/3.0);
            int64_t times = curr_m/trunc(pow(value, 3.0));
            if (value) {
                curr_m -= times*trunc(pow(value, 3.0));
                curr_k += times;
            }
        }
        if (curr_k > best_k) {
    ...
    cout << best_k << ' ' << orig-best_v-added << endl;
    return 0;
}
